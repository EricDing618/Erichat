...
def to_id(ip):
    ...
    for i in ip: #main
        if i in publicDict['number']:
            ...
            make.append(choice(publicDict['.']))
        elif i == ':':
            make.append(choice(publicDict[':']))
    for i in range(8): #salt
        make.insert(randint(0,len(make)),choice(publicDict['saltletterList']))
    return ''.join(make)
...
def to_ip(FCID):
    ...
    make=[]
    for i in FCID:
        if i in publicDict['usefulletterList']:
...
if __name__ == '__main__':
    ...
    print(to_ip('zuCRLoI?en=nK~6zH'))
    print(is_same(['z3uaaoQ!nK@n]z','zIuoU]n?nW]cZz5']))